    ...
    if(!empty($verficateemail->dataaccessmodel->getuser($username, $vertifycode[1]))){
        ...
        if(empty($gebruikermodel->getByGebruikersNaam($username))){
            if($gebruikermodel->Add($username,
                $verficateemail->dataaccessmodel->getuserpassword($username, $vertifycode[1])["Wachtwoord"], $email) &&
                (strtotime(date('m/d/Y h:i:s a', time())) - strtotime($verficateemail->dataaccessmodel->getuser($username, $vertifycode[1])["Timestamp"]) <= 100)){

                  $status = "success";
                  $response = "account aanmelden is gelukt";
                  $verficateemail->dataaccessmodel->delete(($verficateemail->dataaccessmodel->getuser($username, $vertifycode[1])["UserID"]));
            }
            else{
                  $status = "failed";
